...
/*
 * __attribute__
 * weak symbol: It can be overridden by a function of the same name defined elsewhere.
 *              If no other definition exists, the weak one is used.
 * alias("Default_Handler"): unless overridden, the function will run the same code as Default_Handler()
 */
...
/* Initialized the Vector Table */

/*
 * Detailed Vector Table is listed on page 201 of the RM0383 document 
 * Zeros act as placeholders for the positions corresponding to
 * interrupts not supported by our specific mcu (STM32F411RE).
 */
